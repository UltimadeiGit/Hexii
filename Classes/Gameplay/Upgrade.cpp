...
//	m_upgrades.push_back(std::make_shared<Upgrade>(fmt::string_view{ "CriticalChance2" }, fmt::string_view{ "Critical yields are twice as powerful" }, fmt::string_view{ "--" }, 6000, 12));
...
//}
//
//const Upgrades::UpgradeVec& Upgrades::getUpgrades() {
//	return Upgrades::getInstance()->m_upgrades;
//}
//
//const Upgrades::UpgradeVec Upgrades::getUpgradesBetweenLevels(BigInt lower, BigInt higher) {
//	UpgradeVec& fullList = Upgrades::getInstance()->m_upgrades;
//	UpgradeVec subList;
//
//	if (higher < lower) return subList;
//
//	for (unsigned int i = 0; i < fullList.size(); i++) {
//		unsigned int levelRequirement = fullList[i]->levelRequirement;
//		if (levelRequirement >= lower && levelRequirement <= higher) subList.push_back(fullList[i]);
//
//		// No more upgrades will fall in the given range
//		if (levelRequirement > higher) break;
//	}
//
//	return subList;
//}
//
//const Upgrades::UpgradeVec Upgrades::getUpgradesFollowing(UpgradePtr pivot, BigInt count) {
//	UpgradeVec& fullList = Upgrades::getInstance()->m_upgrades;
//	UpgradeVec subList;
//	
