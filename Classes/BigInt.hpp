...
/*
    BigInt ^ BigInt
    ---------------
    Exponentiation by Ultimadei. 
*/

BigInt BigInt::operator^(const BigInt& num) const {
    // I don't know a good algorithm for this and for the purposes of this program, I don't think it's necessary to look one up
    // repeated multiplication should be good enough for these purposes

    BigInt ret = *this;

    // TODO: Just use approximations for values of `num` larger than 100
    if (num > 100 || num < 0) return ret;
    if (num == 0) return 1;
